...
// --- importFootage: Import a file into the AE project ---
function importFootage(args) {
    try {
        ...
        return JSON.stringify({
            ...
            item: { name: importedItem.name, id: importedItem.id, width: importedItem.width, height: importedItem.height, typeName: importedItem.typeName }
        }, null, 2);
    } catch (error) {
        return JSON.stringify({ status: "error", message: error.toString() }, null, 2);
    }
...
// --- addFootageToComp: Add an imported footage item as a layer to a composition ---
function addFootageToComp(args) {
    try {
        var compIndex = parseInt(args.compIndex);
        var itemName = args.itemName; // name of footage in project panel
        var itemId = args.itemId;     // or use id
        var layerName = args.layerName;
        var position = args.position;
        var scale = args.scale;
        var startTime = args.startTime;
        var duration = args.duration;
        var moveToBack = args.moveToBack;

        // Find comp
        var comp = app.project.items[compIndex];
        if (!(comp instanceof CompItem)) throw new Error("Item at index " + compIndex + " is not a composition");

        // Find footage item
        var footageItem = null;
        for (var i = 1; i <= app.project.numItems; i++) {
            var item = app.project.item(i);
            if (itemId && item.id === itemId) { footageItem = item; break; }
            if (itemName && item.name === itemName) { footageItem = item; break; }
        }
        if (!footageItem) throw new Error("Footage item not found: " + (itemName || itemId));

        // Add to comp
        var layer = comp.layers.add(footageItem);
        if (layerName) layer.name = layerName;
        if (position) layer.property("Position").setValue(position);
        if (scale) layer.property("Scale").setValue(scale);
        if (startTime !== undefined && startTime !== null) layer.startTime = startTime;
        if (duration !== undefined && duration !== null && duration > 0) {
            layer.outPoint = (startTime || 0) + duration;
        }
        if (moveToBack) {
        ...
        return JSON.stringify({
            status: "success",
            message: "Footage added to composition",
...
// --- executeCustomScript: Run arbitrary ExtendScript code ---
    ...
    try {
        ...
        switch (command) {
            ...
            case "importFootage":
                ...
                break;
            case "addFootageToComp":
                logToPanel("Calling addFootageToComp function...");
                result = addFootageToComp(args);
                logToPanel("Returned from addFootageToComp.");
                break;
            case "executeCustomScript":
                logToPanel("Calling executeCustomScript function...");
                result = executeCustomScript(args);
                logToPanel("Returned from executeCustomScript.");
                break;
            default:
